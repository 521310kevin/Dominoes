...
class Dominoes(GameAI):
    def __init__(self, gameTiles, myTiles, starter):
        ...
        self.lastPlay = 0
        self.ends = [None, None]
        self.probabilities = {d: (
            [0, 1./3, 1./3, 1./3] if d not in self.myTiles else [1, 0, 0, 0]) for d in self.tiles}
    ...
    def playedTile(self, tile):
        ...
        return sum(map(lambda x: not x == Domino(-1, -1), tile))
    ...
    def isEnd(self):
        ...
        if currentTiles == 0:
            return True
        if self.lastPlay > 3:
            return all(map(lambda x: x == Domino(-1, -1),
                           self.dominos_played[self.lastPlay-3:self.lastPlay+1]))
        return False
        ...
        for d in self.probabilities:
            if d not in self.dominos_played:
                ...
                expectation_opp += value * \
                    probs[(player + 1) % 4] + value*probs[(player + 3) % 4]
                expectation_us += value * \
                    probs[player] + value*probs[(player + 2) % 4]
